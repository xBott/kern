...
public record VersionedDependencyRequest<K>(
    ...
    /// @throws IllegalArgumentException if version format does not satisfy SemVer
    public VersionedDependencyRequest {
        try {
            RangeListFactory.create(versionRange);

        } catch (Exception ex) {
            throw new IllegalArgumentException("Invalid version range: " + versionRange, ex);
